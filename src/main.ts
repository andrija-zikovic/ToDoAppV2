...
const sortButtons: NodeListOf<HTMLButtonElement> | null = document.querySelectorAll('.sort')
...
const sortDate: NodeListOf<HTMLButtonElement> | null = document.querySelectorAll('.sortDate')
...
window.addEventListener('click', (e: Event) => {
  if (e.target !== menuBox && e.target !== menu && e.target !== menu?.children[0]) {
    menuBox?.classList.remove('visible')
    menuBox?.classList.add('invisible')
  }
})
